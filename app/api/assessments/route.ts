...
import { createAssessmentViaSupabase, isPrismaConnectionError, listAssessmentsViaSupabase } from '@/lib/app-db-fallback';
...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      assessments = await listAssessmentsViaSupabase(userId, clientId);
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      if (!isPrismaConnectionError(err)) throw err;
      assessment = await createAssessmentViaSupabase(userId, body);
    }
